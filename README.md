...
メール通知されたイベント情報を自動的に Google カレンダーへ登録し、LINE で結果を通知するアプリケーション。

## 解決する課題

- **カレンダー登録の手間削減**: メール通知から手動登録への手間を自動化
- **スケジュール漏れの防止**: メール通知を見逃してスケジュール漏れを防ぐ
- **非構造化データの抽出検証**: LLMを活用し、メール本文のような非構造化テキストからイベント情報を自動抽出
...
このプロジェクトは **Lambdalith** パターン（単一の Lambda 関数に複数の API エンドポイントを持つ設計） を検証するために実装されています。
...
このアーキテクチャは Lambdalith パターンの検証を目的としていた。  
本アプリケーションレベルだと過剰設計であり、API で分割することなく単一の Lambda 関数で完結させるのが本来は望ましい。
...
- ホストには Docker 以外の依存を要求せず、Python 3.12 / AWS CLI / uv はコンテナ内で利用する。
