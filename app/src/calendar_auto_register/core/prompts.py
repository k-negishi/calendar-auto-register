...
- メール本文に「支払い期限（支払期限/入金期限/決済期限/支払締切）」がある場合は、**主イベントとは別に「支払い期限イベント」を必ず抽出する**
    - クレジットカードなどで自動的に決済の場合は、支払期限イベントを作成しない。目的は、コンビニ払いなどで支払いタスクが発生する場合にカレンダーに登録することであるため。
...
5. 支払い期限がある場合：
   ...
   - 支払い期限用 event を別に生成（終日予定、location禁止、summaryは `支払い期限 HH:MM@イベント名`）
     - ただしクレジットカードなどで自動的に支払われる場合は支払期限用 event は作成禁止。コンビニ払いなど支払タスクが発生する場合のみ event を作成する
6. 日時を正規化：
