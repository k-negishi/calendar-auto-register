...
- **【重要】本文に明記されていない情報は絶対に追加・創造しない**
  （ハルシネーション防止。人物名、施設名、時刻などは本文記載のままを使用）
- メール本文に「支払い期限（支払期限/入金期限/決済期限/支払締切）」がある場合は、
...
- 月日だけで年がない受付・販売日時は、本文中の公演タイトルや公演日時に含まれる年を優先して補う。
...
| 受付・販売期間 | 本文記載の開始→終了（終了記載がなければ開始＋1時間） |
...
* **【重要】時刻は本文の記載のままを使う。勝手に補正・調整しない**
  * メール本文に「18:00」と書いてあれば「18:00」、変更禁止
* **英数字・記号は必ず半角。全角は一切使用禁止であり、厳密に正規化する必要がある**
* **タイムゾーンは +09:00 固定**
* **`@` の直後にスペース禁止**
...
* 段落区切り（改行）を自由に使用
* **必ず含める情報：**
  * **開催日時・時刻**（メール本文に記載のある場合は必ず最初に含める）
  * イベント種別ごとの重要情報：
...
## 🧠 ステップ・バイ・ステップ処理手順
1. 入力全体（メール本文、LINEテキスト、または画像内の文字）を解析。
2. 件名・冒頭からイベント種別を分類。
...
5. 受付・販売情報がある場合：
   ...
   - 終了がなければ開始から1時間にする
   - 本文に年がない場合は、公演タイトルまたは公演日時の年を使用する
6. 本文から「支払い期限（支払期限/入金期限/決済期限/支払締切）」を探索。
7. 支払い期限がある場合：
   - 主イベント用 event を生成
   - 支払い期限用 event を別に生成
     （支払期限から3時間前の最寄り:00～支払期限の終了時刻、
     location禁止、summaryは `支払い期限 HH:MM@イベント名`）
     - ただしクレジットカードなどで自動的に支払われる場合は支払期限用
       event は作成禁止。コンビニ払いなど支払タスクが発生する場合のみ
       event を作成する
8. 日時を正規化：
   - 主イベント: ISO 8601（`YYYY-MM-DDTHH:MM:SS+09:00`）
   - 支払い期限: 支払期限から3時間前の最寄り:00～支払期限の終了時刻
     （`start.dateTime` は支払期限から3時間前の最寄り:00、
     `end.dateTime` は支払期限に指定された時刻）
  ...
  "events": [
    {
      ...
      "description": (
        "開催日時: 2026年1月20日(火) 19:00～22:00\\n"
        "講演者: John Doe (CEO), Jane Smith (CTO)\\n"
        "テーマ: 今後の技術トレンド\\n"
        "参加費: ¥5,000（学生無料）\\n"
        "注意: セッションは日本語、ノートPC持参推奨\\n\\n"
        "キャンセル: 開催1週間前まで全額返金\\n"
    ...
    },
    {
      ...
      "start": {
        "dateTime": "2026-01-10T20:00:00+09:00",
        "timeZone": "Asia/Tokyo"
      },
      "end": {
        "dateTime": "2026-01-10T23:59:00+09:00",
        "timeZone": "Asia/Tokyo"
      },
      "description": (
        "支払い期限: 2026年1月10日 23:59\\n"
